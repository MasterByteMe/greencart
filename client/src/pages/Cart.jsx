...
const Cart = () => {
    ...
    const getCart = () => {
        // Initialize an empty temporary array to store cart items with quantities
        let tempArray = [];

        // Loop through each key in the cartItems object
        // Each key represents a product ID
        for (const key in cartItems) {
            // Find the product in the products array that matches the current key (product ID)
            const product = products.find((item) => item._id === key);

            // Add the quantity from cartItems to the product object
            // This attaches how many units of this product are in the cart
            product.quantity = cartItems[key];

            // Add the modified product (with quantity) to our temporary array
            tempArray.push(product);
        }

        // Update the state variable cartArray with the fully processed cart items
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddress ? `${selectedAddress.street},${selectedAddress.city},${selectedAddress.state},${selectedAddress.country}` : "No address found"}
                        </p>
